...
You just declared a record whose name is "Person". This record has 2 variables, 2 **components**: `lastname` and `firstname`.
...
**2. Recaord's acquired standard members**
...
You can now use `javap` to decompile the record class, and see the different standard members that were acquired automatically by the "Person" record.
...
* 2 private final fields (`lastname` and `firstname`). Those match the record's components declared in its header. They are immutable.
* a canonical constructor taking 2 parameters that match, again, the Record's components (`lastname` and `firstname`).
...
All of that is generated by the Java compiler. Using Records, we can declare transparent data carrier classes using a clean and concise syntax. It is also possible to override those methods implementation or supply our a custom constructor (ex. to perform data validation and normalization).
...
Finally, records can also implement interface(s), ex. the `java.io.Serializable` interface. And regarding that topic, i.e. serialization, it should be mentioned that records use a specific, well defined, serialization protocol that is particularly safe as 
...
Records allow to easily and quickly create immutable data aggregates. Records went through 2 preview rounds (Java 14 & Java 15) and are slated to be made final and permanent in Java 16.
